...
is_html( $form->field('no_render')->render, '', 'no_render' );

is_html( $form->field('plain')->render, '<input type="text" name="plain" id="plain" value="No divs!!" />', 'renders without wrapper');

is_html( $form->field('boxed')->render, '<fieldset class="boxed"><legend>Boxed</legend><input type="text" name="boxed" id="boxed" value="Testing single fieldset" /></fieldset>
...

{
    ...
    extends 'HTML::FormHandler';
    with 'MyApp::Form::Widget::Form::Simple';
    has '+widget_name_space' => ( default => sub {['Widget', 'MyApp::Form::Widget']} );
...
is_html( $form->field('alpha')->render, '<p>The test succeeded.</p>', 'alpha rendered ok' );
is_html( $form->field('omega')->render, '<h1>You got here!</h1>',     'omega rendered ok' );
is_html( $form->field('gamma')->render, '<p>The test succeeded.</p>', 'alpha rendered ok' );
...
is_html( $out, $expected, 'compound field with inline wrapper' );
...
is_html( $out, $expected, 'compound field with table inline wrapper' );
